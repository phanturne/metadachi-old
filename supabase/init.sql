...
DROP POLICY IF EXISTS "Everyone can select their own and public prompts" on Prompts;
CREATE POLICY "Everyone can select their own and public prompts" ON Prompts
FOR SELECT USING (public = TRUE OR (SELECT auth.uid()) = user_id);

DROP POLICY IF EXISTS "Authenticated users can insert their own prompts" on Prompts;
CREATE POLICY "Authenticated users can insert their own prompts" ON Prompts
FOR INSERT TO authenticated WITH CHECK ((SELECT auth.uid()) = user_id);

DROP POLICY IF EXISTS "Authenticated users can update their own prompts" on Prompts;
CREATE POLICY "Authenticated users can update their own prompts" ON Prompts
FOR UPDATE TO authenticated USING ((SELECT auth.uid()) = user_id) WITH CHECK ((SELECT auth.uid()) = user_id);

DROP POLICY IF EXISTS "Authenticated users can delete their own prompts" on Prompts;
CREATE POLICY "Authenticated users can delete their own prompts" ON Prompts
FOR DELETE TO authenticated USING ((SELECT auth.uid()) = user_id);
